...
describe('EmployeeSessionVerificationService (ADR-0150/ADR-0157)', () => {
  function buildService(sessionEmployeeId: string | null) {
    ...
    const service = new EmployeeSessionVerificationService(employeeGrpcClient as never);
    return { service, employeeGrpcClient };
  }
  ...
  it('throws ForbiddenException when the requested employeeId does not match the session', async () => {
    const { service } = buildService(EMPLOYEE_ID);
  ...
  it('throws ForbiddenException when the token has no sub claim', async () => {
    const { service, employeeGrpcClient } = buildService(EMPLOYEE_ID);
