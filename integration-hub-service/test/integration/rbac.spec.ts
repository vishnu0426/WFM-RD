...
/**
 * §7 Phase 8's real end-to-end verification of the RBAC guard trio
 * (ADR-0145, own copy of ai-layer-service's ADR-0130 guards) - a real
 * local HTTP server serving a real JWKS (a freshly generated RSA keypair),
 * real signed JWTs (`jose.SignJWT`), and the actual `AccessTokenGuard`/
 * `PermissionsGuard`/`TenantTokenMatchGuard` classes this module's
 * resolvers use - not a reimplementation or a mock of their logic. Only
 * the NestJS `ExecutionContext` itself is a minimal hand-built stand-in
 * exposing exactly the two methods (`getType`, `switchToHttp().getRequest`,
 * `getHandler`) these guards actually call - standard practice for testing
 * a Guard in isolation, not a shortcut around the guard's own real
 * behavior.
 */
describe('RBAC guard trio (real JWKS server + real signed JWTs)', () => {
  ...
  it('a real valid token with the correct permission and matching tenant passes all three guards', async () => {
    const token = await signToken(privateKey, { tenant_id: tenantId, permissions: ['integration_connector:write'] });
  ...
  it('rejects a real token signed by the wrong key', async () => {
    const token = await signToken(wrongPrivateKey, {
      tenant_id: tenantId,
      permissions: ['integration_connector:write'],
    });
    const req: RequestWithTokenClaims = { headers: { authorization: `Bearer ${token}` } } as never;
    await expect(accessTokenGuard.canActivate(makeHttpContext(req, GatedTarget.prototype.gated))).rejects.toThrow(
      UnauthorizedException,
  ...
  it('rejects a real expired token', async () => {
    const token = await signToken(
      privateKey,
      { tenant_id: tenantId, permissions: ['integration_connector:write'] },
  ...
  it('PermissionsGuard rejects a valid token missing the required permission', async () => {
    const token = await signToken(privateKey, { tenant_id: tenantId, permissions: ['webhook_subscription:write'] });
    const req: RequestWithTokenClaims = { headers: { authorization: `Bearer ${token}` } } as never;
