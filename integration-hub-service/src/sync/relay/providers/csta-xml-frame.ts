...
/** The Invoke ID Annex J §J.2 reserves for unsolicited (server -> client) CSTA events, never used on a request this adapter sends. */
...
export function encodeCstaFrame(invokeId: string, xmlBody: string): Buffer {
  if (invokeId.length !== INVOKE_ID_LENGTH) {
    throw new Error(`CSTA Invoke ID must be exactly ${INVOKE_ID_LENGTH} ASCII characters, got "${invokeId}"`);
  }
  ...
  return Buffer.concat([Buffer.from([0x00, 0x00]), lengthBuf, invokeBuf, xmlBuf]);
}
    ...
    for (;;) {
      ...
      if (isPlainCstaXml) {
        const invokeId = this.buffer
          ...
          .toString('ascii');
        const xml = this.buffer.subarray(FRAME_PREFIX_LENGTH + INVOKE_ID_LENGTH, totalLength).toString('ascii');
        frames.push({ invokeId, xml });
      }
      // A SOAP-enveloped frame (header `0x01`) is well-formed per the
      // standard but not a shape this adapter speaks - dropped rather than
      // mis-decoded as plain CSTA-XML.
