...
/**
 * ADR-0163's real end-to-end verification of this module's own first RBAC
 * guard trio - own copy of adherence-compliance-service's identical test
 * (their ADR-0161, itself a copy of integration-hub-service's/
 * ai-layer-service's ADR-0145/ADR-0130). A real local HTTP server serving a
 * real JWKS (a freshly generated RSA keypair), real signed JWTs
 * (`jose.SignJWT`), and the actual `AccessTokenGuard`/`PermissionsGuard`/
 * `TenantTokenMatchGuard` classes `DashboardResolver` now uses - not a
...
describe('RBAC guard trio (real JWKS server + real signed JWTs)', () => {
  ...
  async function signToken(
    key: KeyLike,
    claims: { tenant_id: string; permissions: string[]; roles?: string[]; sub?: string },
    overrides?: { issuer?: string; audience?: string; expiredSecondsAgo?: number },
  ): Promise<string> {
    const jwt = new SignJWT({ roles: [], ...claims })
  ...
  it('a real valid token with the correct permission and matching tenant passes all three guards, carrying its roles claim through', async () => {
    ...
    expect(req.tokenClaims?.roles).toEqual(['manager', 'analyst']);
  ...
  it('rejects a real token signed by the wrong key', async () => {
    const token = await signToken(wrongPrivateKey, { tenant_id: tenantId, permissions: ['dashboard:write'] });
    const req: RequestWithTokenClaims = { headers: { authorization: `Bearer ${token}` } } as never;
    await expect(accessTokenGuard.canActivate(makeHttpContext(req, GatedTarget.prototype.gated))).rejects.toThrow(
      UnauthorizedException,
    );
  ...
  it('rejects a real expired token', async () => {
    const token = await signToken(
      privateKey,
      { tenant_id: tenantId, permissions: ['dashboard:write'] },
  ...
  it('PermissionsGuard rejects a valid token missing the required permission', async () => {
    const token = await signToken(privateKey, { tenant_id: tenantId, permissions: ['dashboard:read'] });
    const req: RequestWithTokenClaims = { headers: { authorization: `Bearer ${token}` } } as never;
  ...
  it('TenantTokenMatchGuard rejects a valid token whose tenant does not match the request tenant context', async () => {
    const token = await signToken(privateKey, { tenant_id: tenantId, permissions: ['dashboard:write'] });
    const req: RequestWithTokenClaims = { headers: { authorization: `Bearer ${token}` } } as never;
    await accessTokenGuard.canActivate(makeHttpContext(req, GatedTarget.prototype.gated));
