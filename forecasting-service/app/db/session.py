"""Python equivalent of `with-tenant-transaction.ts` (ADR-0002): every
request-scoped unit of work opens one transaction and, as its *first*
statement, binds `app.current_tenant_id`/`app.is_platform_admin` via
`set_config(..., true)` (the `true` third argument is Postgres's "local to
this transaction" flag - the parameterized equivalent of `SET LOCAL`, exactly
as the Node comment notes `SET LOCAL` itself doesn't accept bind
parameters). Both values must already be validated/sourced from
`TenantContext` - never from client-supplied input beyond what
`app/api/deps.py`'s placeholder header-read already accepts (ADR-0014's same
caveat applies here).
"""
...
async def tenant_scoped_session(
    ...
    *,
    ...
    """The only way application code should touch `forecasting.*` tables -
