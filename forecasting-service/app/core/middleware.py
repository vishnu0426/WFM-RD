...
`tenant_id` claims this middleware now reads).

ADR-0026, Decision 7: this is also where structured request logging happens
(`log_request`, `app/core/logging_config.py`) - deliberately inline here,
not a separate middleware layer. An outer middleware logging *after*
`call_next()` returns would see `tenant_id` already unbound (the `with
bind(context):` block below has already exited by the time control returns
to anything outside it) - logging from inside the same function that calls
`bind()` sidesteps that ordering hazard entirely rather than fighting
...
class TenantContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        if request.url.path in _UNVERIFIED_ROUTES:
            ...
            self._log(request, response, start, tenant_id=None)
            return response
        ...
        with bind(context):
            response = await call_next(request)
            self._log(request, response, start, tenant_id=context.tenant_id)
            return response

    @staticmethod
    def _log(request: Request, response: Response, start: float, *, tenant_id: str | None) -> None:
        log_request(
