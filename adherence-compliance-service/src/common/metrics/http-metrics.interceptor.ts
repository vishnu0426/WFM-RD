...
/**
 * Phase 2: multi-transport, mirroring intraday-service's own
 * `HttpMetricsInterceptor` - this class's own Phase 1 doc comment predicted
 * exactly this upgrade the instant a GraphQL surface landed
 * (`context.switchToHttp().getRequest()` doesn't return a real Express
 * `Request` in a GraphQL context; caught by this phase's own real-boot
 * verification via `curl`, not by a type system that had no way to know).
 *
 ...
 * `<Type>.<field>` (e.g. `Mutation.createComplianceRule`) for GraphQL -
 * both bounded-cardinality label shapes, matching Prometheus's own best
 * practices.
 */
@Injectable()
