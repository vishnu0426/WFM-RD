...
/**
 * ADR-0161: own copy of integration-hub-service's/ai-layer-service's
 * identical Bearer-token guard (ADR-0145/ADR-0130) - this module's own
 * first RBAC infrastructure, this service having previously had none at
 * all (every write open to any caller with a valid tenant header).
 *
 ...
 *   completely separate Postgres role/schema, ADR-0093). `jose`'s
 ...
 *   are short-lived (720s, core's own `token.service.ts`), the standard
 *   trade-off most OIDC resource servers make rather than requiring a
 *   shared revocation cache with every downstream service.
 */
@Injectable()
