...
describe('AccessTokenGuard', () => {
  ...
  async function signToken(
    ...
    return new SignJWT({ tenant_id: 'tenant-a', permissions: ['marketplace_claim:approve'], sub: 'user-1' })
  ...
  it('accepts a genuine token signed by a key the real JWKS endpoint actually serves', async () => {
    ...
    expect(request.tokenClaims).toMatchObject({ tenant_id: 'tenant-a', permissions: ['marketplace_claim:approve'] });
  });

  it('rejects a request with no Authorization header and records an RBAC-denial metric', async () => {
