...
/**
 * Own copy of intraday-service's `json.scalar.ts`, for `MarketplaceClaim.
 * validationResult` (§4 step 2) and `Bid.rankExplanation` (§5.1) - both
 * shapes vary by category/ranking_method, so a typed GraphQL object isn't
 * a fit, same reasoning as root's `Policy.definition`. Hand-rolled rather
 * than `graphql-type-json`: the full behavior needed is "pass an arbitrary
