...
// check - same deliberately permissive pattern intraday-service's own copy
// uses, for the same reason: this service's dev/test tenant ids don't set
// valid variant bits and would be wrongly rejected by the stricter form.
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/**
 * Own copy of the root app's `TenantContextService` (ADR-0039 precedent:
 ...
 * exists now, real and tested, so Phase 2's ingestion controller and
 * Phase 3's leave-request mutations have a proven context primitive to
 * bind against rather than inventing it under phase pressure later.
 */
@Injectable()
